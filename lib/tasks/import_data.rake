  ...
  task google_fit_weight_json: :environment do
    ...
    ActiveRecord::Base.transaction do
      data_hash['point'].each do |point|
        date = Time.at(point['startTimeNanos'].to_i / 1_000_000_000).to_date
        weight_pounds = point['value'][0]['fpVal'].to_f * 2.20462
        unless WeightReport.exists?(date: date)
          WeightReport.create!(date: date, weight_pounds: weight_pounds)
        end
      end
    end
  end
